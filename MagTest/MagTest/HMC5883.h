/**
@file HMC5883.h - Interface a Honeywell HMC5883 magnetometer to an Arduino via i2c.
...
#define HMC5883_ADDR 0x0E // 7 bit address of the HMC5883 used with the Wire library
...
#ifdef ISHMC5843
    #define HMC5883_R_YM (5)    //!< Register address for YM.
    #define HMC5883_R_YL (6)    //!< Register address for YL.
    #define HMC5883_R_ZM (7)    //!< Register address for ZM.
    #define HMC5883_R_ZL (8)    //!< Register address for ZL.

    #define HMC5883_X_SELF_TEST_GAUSS (+0.55)                       //!< X axis level when bias current is applied.
    #define HMC5883_Y_SELF_TEST_GAUSS (HMC5883_X_SELF_TEST_GAUSS)   //!< Y axis level when bias current is applied.
    #define HMC5883_Z_SELF_TEST_GAUSS (HMC5883_X_SELF_TEST_GAUSS)   //!< Y axis level when bias current is applied.

    /*
        This is my best guess at the LOW, HIGH limit.  The data sheet does not have these values.
    */
    #define SELF_TEST_LOW_LIMIT  (HMC5883_X_SELF_TEST_GAUSS*0.53)   //!< Low limit 53% of expected value.
    #define SELF_TEST_HIGH_LIMIT (HMC5883_X_SELF_TEST_GAUSS*1.36)   //!< High limit 136% of expected values.
#else // HMC5883L
...
class HMC5883
{
  public:
    ...
    void getHMCIntValues(int *x,int *y,int *z);
    void getHMCFloatValues(float *x,float *y,float *z);
